...
func parseSignatureBond(id ecc.ID, buf []byte) ([]byte, []byte, []byte, []byte) {
...
func parsePointBond(id ecc.ID, buf []byte) ([]byte, []byte) {
	var pointbn254 edwardsbn254.PointAffine
...
func TestBondv(t *testing.T) {
	...
	*  First step: Compile and Setup circuit.
	 */
	var circuit libgnark.BondCircuit
	...
	* Populate test cases
	...
	for i := 0; i < size; i++ {
		...
		* Hash and Signatures
		...
		// Seting up
		var witness libgnark.BondCircuit
		...
		sigRxt, sigRyt, sigS1t, sigS2t := parseSignatureBond(id, BondQuoteSignedCpt1)
		...
		sigRxt, sigRyt, sigS1t, sigS2t = parseSignatureBond(id, BondQuoteSignedCpt2)
		...
		sigRxt, sigRyt, sigS1t, sigS2t = parseSignatureBond(id, BondQuoteSignedCpt3)
		...
		sigRx, sigRy, sigS1, sigS2 := parseSignatureBond(id, AcceptedQuoteSigned)
		...
		//A
		pubkeyAx, pubkeyAy := parsePointBond(id, pubKeyCpt1.Bytes())
		...
		sigRx, sigRy, sigS1, sigS2 = parseSignatureBond(id, signatureCpt1)
		...
		//B
		pubkeyBAx, pubkeyBAy := parsePointBond(id, pubKeyBCpt2.Bytes())
		...
		sigBRx, sigBRy, sigBS1, sigBS2 := parseSignatureBond(id, signatureCpt2)
		...
		//C
		pubkeyCAx, pubkeyCAy := parsePointBond(id, pubKeyCpt3.Bytes())
		...
		sigCRx, sigCRy, sigCS1, sigCS2 := parseSignatureBond(id, signatureCpt3)
		...
		if err != nil {
			...
			fmt.Println("Test", i, "fails proof")

		} else {

			//Check with a correct value and it returns NIL
			var witnessCorrectValue libgnark.BondCircuit
