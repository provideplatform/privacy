...
type BondCircuit struct {
	...
	SignatureCpt1       Signature         `gnark:",secret"` // Sign(quote)
	SignatureCpt2       Signature         `gnark:",secret"` // Sign(quote)
	SignatureCpt3       Signature         `gnark:",secret"` // Sign(quote)
	QuoteFromCpt1       frontend.Variable `gnark:",secret"` // Example: 92.63
	QuoteFromCpt2       frontend.Variable `gnark:",secret"` // Example: 92.40
	QuoteFromCpt3       frontend.Variable `gnark:",secret"` // Example: 93
	AcceptedQuotePubKey PublicKey         `gnark:",secret"` // It is going to be PublicKeyCpt1 or PublicKeyCpt2 or PublicKeyC
	AcceptedQuote       frontend.Variable `gnark:",secret"` // ig: 92.63
	RejectedQuote1      frontend.Variable `gnark:",secret"` // ig: 93 - Rejected quote order doesn't matter
	RejectedQuote2      frontend.Variable `gnark:",secret"` // ig: 92.40
	BondQuoteSignedCpt1 Signature         `gnark:",secret"` // Sign(Bond hash, quote)
	BondQuoteSignedCpt2 Signature         `gnark:",secret"` // Sign(Bond hash, quote)
	BondQuoteSignedCpt3 Signature         `gnark:",secret"` // Sign(Bond hash, quote)
}
